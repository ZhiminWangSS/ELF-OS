...
def main():
    parser = argparse.ArgumentParser(description="ELF-OS model deployment")
    sub = parser.add_subparsers(dest="command", required=True)
    run = sub.add_parser("run")
    run.add_argument("--model", default="seekvln-4k-sft")
    ...
    run.add_argument("--run-dir", type=Path, default=Path("runs/model-deployment"))
    run.add_argument("--execute", action="store_true")
    args = parser.parse_args()
    request = SubgoalRequest(subgoal_id=args.subgoal_id, instruction=args.instruction, model_id=args.model,
